...
static void replaceLibPathInEnvBlock(JNIEnv *env, jbyteArray* envBlock, jint* envc, const char* directory) {
    ...
    char envStr[1024];
    jsize new_envl = snprintf(envStr, sizeof(envStr) / sizeof(char), "LD_LIBRARY_PATH=%s%cLD_PRELOAD=libnativewindow.so:libandroid.so:libcutils.so%cPATH=%s", directory, 0, 0, directory) + 1;
