...
static void replaceLibPathInEnvBlock(JNIEnv *env, jbyteArray* envBlock, jint* envc, const char* directory) {
    ...
    char envStr[1024];
    jsize new_envl = snprintf(envStr, sizeof(envStr) / sizeof(char), "LD_LIBRARY_PATH=%s:/system/lib64:/vendor/lib64%cLD_PRELOAD=libnativewindow.so:libcutils.so:libc++_shared.so:libandroid.so%cPATH=%s", directory, 0, 0, directory) + 1;
